...
def show_tracked_img(img_dict, ip_set, num_matched, output_video, args):
    ...
    img = write_on_image(img=img, text=tagged_df["text"], #no슬라이싱
...
def alg2_sequential(queues, argss, consecutive_frames, event,drone):
    fall_count = 0
    ...
    while True:
        ...
        if not any(q.empty() for q in queues):
            ...
            if argss[0].num_cams == 1:
                ...
                if dict_frames[0]["tagged_df"]["text"].endswith("FALL"):
                    fall_count += 1
                    if fall_count >= 100:
                        sd.Beep(1000, 100)  # 1000Hz 소리를 0.1초 동안 재생
                else:
                    fall_count = 0
